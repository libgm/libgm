...
namespace libgm { namespace experimental {
  ...
  public:
    ...
    bool alias(const real_vector<real_type>& param) const {
      ...
      // which returns a real_matrix temporary and thus acts as a shield
  ...
  outer_join(JoinOp join_op,
    ...
    return make_matrix_function<Space>(
      [join_op](const Derived& f, const Other& g, real_matrix<RealType>& result) {
        auto&& u = f.param();
        auto&& v = g.param();
  ...
  inline auto matrix_from_table(const F& f) {
    ...
    return make_matrix_function<Space>(
      [](const F& f, real_matrix<typename F::real_type>& result) {
        auto&& param = f.param();
