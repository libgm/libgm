...
#include <libgm/learning/dataset/vector_dataset_io.hpp>
...
typedef std::pair<vector_assignment<>, double> sample_assignment_type;
...
BOOST_AUTO_TEST_CASE(test_value_iterators) {
  ...
  vector_dataset<>::iterator it1, end1;
  std::tie(it1, end1) = ds(v);

  vector_dataset<>::const_iterator it2 = ds.begin();
  vector_dataset<>::const_iterator end2 = ds.end();
...
BOOST_AUTO_TEST_CASE(test_assignment_iterators) {
  ...
  vector_dataset<>::assignment_iterator it, end;
