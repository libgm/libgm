...
#include <libgm/factor/experimental/expression/matrix_vector_join.hpp>
#include <libgm/factor/experimental/expression/vector_matrix_join.hpp>
  ...
  private:
    ...
    auto eliminate(AggOp agg_op, int_constant<Eigen::Vertical>) const {
      return make_vector_function<Space>(
        [agg_op](const F& f, real_vector<real_type>& result) {
    ...
    auto eliminate(AggOp agg_op, int_constant<Eigen::BothDirections>) const {
      return make_vector_function<Space>(
        [agg_op, dim = dim_](const F& f, real_vector<real_type>& result) {
          if (dim == 0) {
            result = agg_op(f.param().colwise()).transpose();
          } else {
            result = agg_op(f.param().rowwise());
