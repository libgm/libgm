 ...
 *
 * See also the template specialization VectorProcessor<Job, void> for
...
public:
   ...
   */
  explicit VectorProcessor(std::function<void(const Job&, State&)> fn)
    : fn_(std::move(fn)) { }
...
class VectorProcessor<Job, void> {
public:
  /**
   * Constructs a vector processor with the given processing function.
   */
  VectorProcessor(std::function<void(const Job&)> fn)
    : fn_(std::move(fn)) { }
