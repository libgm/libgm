...
#define LIBGM_TRANSFORM_LEFT(function, factor, scalar, op)              \
  friend auto function(const factor##_base<RealType, Derived>& f,       \
                       scalar x) {                                      \
    return f.derived().transform(op);                                   \
  }                                                                     \
                                                                        \
  friend auto function(factor##_base<RealType, Derived>&& f,            \
                       scalar x) {                                      \
...
#define LIBGM_TRANSFORM_RIGHT(function, factor, scalar, op)             \
  friend auto function(scalar x,                                        \
                       const factor##_base<RealType, Derived>& f) {     \
    return f.derived().transform(op);                                   \
  }                                                                     \
                                                                        \
  friend auto function(scalar x,                                        \
                       factor##_base<RealType, Derived>&& f) {          \
    return std::move(f).derived().transform(op);                        \
  }
...
#define LIBGM_TRANSFORM(function, factor, op)                           \
  ...
  friend auto function(const factor##_base<RealType, Derived>& f,       \
                       const factor##_base<RealType, Other>& g) {       \
...
#define LIBGM_JOIN2(function, factor1, factor2, op)                     \
  template <typename Other>                                             \
  friend auto function(const factor1##_base<RealType, Derived>& f,      \
                       const factor2##_base<RealType, Other>& g) {      \
  ...
  template <typename Other>                                             \
  friend auto function(const factor1##_base<RealType, Derived>& f,      \
                       factor2##_base<RealType, Other>&& g) {           \
    return join(op, f.derived(), std::move(g).derived(),                \
                static_cast<Derived*>(0), static_cast<Other*>(0));      \
  }                                                                     \
                                                                        \
  template <typename Other>                                             \
  friend auto function(factor1##_base<RealType, Derived>&& f,           \
                       const factor2##_base<RealType, Other>& g) {      \
...
#define LIBGM_JOIN2R(function, factor1, factor2, op)                    \
  template <typename Other>                                             \
  friend auto function(const factor1##_base<RealType, Other>& f,        \
                       const factor2##_base<RealType, Derived>& g) {    \
    return join(op, f.derived(), g.derived(),                           \
  ...
  template <typename Other>                                             \
  friend auto function(const factor1##_base<RealType, Other>& f,        \
                       factor2##_base<RealType, Derived>&& g) {         \
    return join(op, f.derived(), std::move(g).derived(),                \
                static_cast<Other*>(0), static_cast<Derived*>(0));      \
  }                                                                     \
                                                                        \
  template <typename Other>                                             \
  friend auto function(factor1##_base<RealType, Other>&& f,             \
                       const factor2##_base<RealType, Derived>& g) {    \
    return join(op, std::move(f).derived(), g.derived(),                \
                static_cast<Other*>(0), static_cast<Derived*>(0));      \
  }                                                                     \
