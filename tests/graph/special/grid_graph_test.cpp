...
BOOST_AUTO_TEST_CASE(creates_expected_grid_connectivity) {
  ...
  std::function<Arg(size_t, size_t)> make_arg = [&](size_t row, size_t col) {
    Arg u = NamedFactory::default_factory().make(
        "(" + std::to_string(row) + ", " + std::to_string(col) + ")");
    args[row][col] = u;
    return u;
  };

  MarkovNetworkT<int, double> graph = make_grid_graph<int, double>(3, 4, make_arg);
...
BOOST_AUTO_TEST_CASE(empty_when_dimension_is_zero) {
  ...
  MarkovNetworkT<int, int> graph1 = make_grid_graph<int, int>(0, 5, make_arg);
  MarkovNetworkT<int, int> graph2 = make_grid_graph<int, int>(4, 0, make_arg);
