...
public:
   ...
   */
  explicit LogarithmicMatrixLL(const DenseMatrix<T>& l)
    : l_(l) { }
   ...
   */
  real_pair<T>
  value_slope(std::size_t i, std::size_t j, const DenseMatrix<T>& dir) const {
   ...
   */
  real_pair<T>
  value_slope(index_pair x, const DenseMatrix<T>& dir) const {
   ...
   */
  real_pair<T>
  value_slope(const uint_vector& x, const DenseMatrix<T>& dir) const {
    assert(x.size() == 2);
   ...
   * point with weight w to the gradient array g
   */
  void add_gradient(index_pair x, T w, DenseMatrix<T>& g) const {
   ...
   * point with weight w to the gradient array g
   */
  void add_gradient(const uint_vector& x, T w, DenseMatrix<T>& g) const {
    assert(x.size() == 2);
   ...
   * data point with weight w to the Hessian diagonal h.
   */
  void add_hessian_diag(index_pair x, T w, DenseMatrix<T>& h) const { }

  /**
   * Adds the diagonal of the Hessian of log-likelihood of the specified
   * data point with weight w to the Hessian diagonal h.
   */
  void add_hessian_diag(const uint_vector& x, T w, DenseMatrix<T>& h) const {
    assert(x.size() == 2);
  }
   ...
   * \param j the column index
   */
  void add_hessian_diag(const DenseMatrix<T>& phead, std::size_t j, T w,
                        param_type& h) const { }

private:
  /// The parameters at which we evaluate the log-likelihood derivatives.
  const DenseMatrix<T>& l_;
