...
#include <libgm/learning/dataset/vector_sequence_dataset_io.hpp>

#include <random>

namespace libgm {
  template class basic_sequence_dataset<vector_sequence_traits<double, variable> >;
  template class basic_sequence_dataset<vector_sequence_traits<float, variable> >;
}
...
struct fixture {
  ...
  fixture()
    ...
    p[0] = u.new_vector_dprocess("a", 1);
    p[1] = u.new_vector_dprocess("b", 2);
    seq0 << 0, 1, 1, 2, 2, 3;
    seq1 << 1, 2, 3;
    ds.initialize(p);
...
BOOST_FIXTURE_TEST_CASE(test_weight_iterator, fixture) {
  vector_sequence_dataset<>::weight_iterator it, end;
