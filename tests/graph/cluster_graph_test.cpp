#define BOOST_TEST_MODULE cluster_graph
#include <boost/test/unit_test.hpp>

#include <libgm/argument/named_argument.hpp>
#include <libgm/graph/algorithm/elimination_strategies.hpp>
#include <libgm/graph/cluster_graph.hpp>

#include <algorithm>
#include <set>
#include <unordered_set>
#include <utility>
#include <vector>

using namespace libgm;

namespace {

Arg make_arg(const char* name) {
  return NamedFactory::default_factory().make(name);
}

struct Fixture {
  using vertex_descriptor = ClusterGraph::vertex_descriptor;
  using edge_descriptor = ClusterGraph::edge_descriptor;

  Fixture() {
    a = make_arg("a");
    b = make_arg("b");
    c = make_arg("c");
    d = make_arg("d");
    e = make_arg("e");
    f = make_arg("f");

    v1 = cg.add_vertex({a, b});
    v2 = cg.add_vertex({b, c, d});
    v3 = cg.add_vertex({c, d, e});
    v4 = cg.add_vertex({d, f});

    e12 = cg.add_edge(v1, v2);
    e23 = cg.add_edge(v2, v3);
    e24 = cg.add_edge(v2, v4);
  }

  Arg a;
  Arg b;
  Arg c;
  Arg d;
  Arg e;
  Arg f;

  ClusterGraph cg;
  vertex_descriptor v1 = nullptr;
  vertex_descriptor v2 = nullptr;
  vertex_descriptor v3 = nullptr;
  vertex_descriptor v4 = nullptr;
  edge_descriptor e12;
  edge_descriptor e23;
  edge_descriptor e24;
};

} // namespace

BOOST_AUTO_TEST_CASE(test_constructors_and_copy_move) {
  ClusterGraph g1;
  BOOST_CHECK(g1.empty());
  BOOST_CHECK_EQUAL(g1.num_vertices(), 0);
  BOOST_CHECK_EQUAL(g1.num_edges(), 0);

  Arg a = make_arg("ca");
  Arg b = make_arg("cb");

  auto* v1 = g1.add_vertex({a});
  auto* v2 = g1.add_vertex({a, b});
  auto e12 = g1.add_edge(v1, v2);

  BOOST_CHECK(!g1.empty());
  BOOST_CHECK(g1.contains(v1));
  BOOST_CHECK(g1.contains(v2));
  BOOST_CHECK(g1.contains(e12));
  BOOST_CHECK_EQUAL(g1.num_vertices(), 2);
  BOOST_CHECK_EQUAL(g1.num_edges(), 1);

  ClusterGraph g2(g1);
  BOOST_CHECK_EQUAL(g2.num_vertices(), 2);
  BOOST_CHECK_EQUAL(g2.num_edges(), 1);
  BOOST_CHECK(g2.is_connected());
  BOOST_CHECK(g2.is_tree());
  BOOST_CHECK(g2.has_running_intersection());

  ClusterGraph g3;
  g3 = g1;
  BOOST_CHECK_EQUAL(g3.num_vertices(), 2);
  BOOST_CHECK_EQUAL(g3.num_edges(), 1);

  ClusterGraph g4(std::move(g3));
  BOOST_CHECK_EQUAL(g4.num_vertices(), 2);
  BOOST_CHECK_EQUAL(g4.num_edges(), 1);

  ClusterGraph g5;
  g5 = std::move(g4);
  BOOST_CHECK_EQUAL(g5.num_vertices(), 2);
  BOOST_CHECK_EQUAL(g5.num_edges(), 1);
}

BOOST_FIXTURE_TEST_CASE(test_graph_accessors, Fixture) {
  BOOST_CHECK(cg.contains(v1));
  BOOST_CHECK(cg.contains(v2));
  BOOST_CHECK(cg.contains(v3));
  BOOST_CHECK(cg.contains(v4));
  BOOST_CHECK(cg.contains(e12));
  BOOST_CHECK(cg.contains(e23));
  BOOST_CHECK(cg.contains(e24));

  BOOST_CHECK(cg.contains(cg.root()));

  BOOST_CHECK_EQUAL(cg.num_vertices(), 4);
  BOOST_CHECK_EQUAL(cg.num_edges(), 3);

  BOOST_CHECK_EQUAL(cg.in_degree(v2), 3);
  BOOST_CHECK_EQUAL(cg.out_degree(v2), 3);
  BOOST_CHECK_EQUAL(cg.degree(v2), 3);

  std::unordered_set<vertex_descriptor> neighbors = {v1, v3, v4};
  for (auto* v : cg.adjacent_vertices(v2)) {
    BOOST_CHECK_EQUAL(neighbors.erase(v), 1);
  }
  BOOST_CHECK(neighbors.empty());

  std::unordered_set<vertex_descriptor> out_targets = {v1, v3, v4};
  for (auto e : cg.out_edges(v2)) {
    BOOST_CHECK(cg.contains(e));
    BOOST_CHECK_EQUAL(out_targets.erase(e.target()), 1);
  }
  BOOST_CHECK(out_targets.empty());

  std::unordered_set<vertex_descriptor> in_sources = {v1, v3, v4};
  for (auto e : cg.in_edges(v2)) {
    BOOST_CHECK(cg.contains(e));
    BOOST_CHECK_EQUAL(in_sources.erase(e.source()), 1);
  }
  BOOST_CHECK(in_sources.empty());

  size_t edge_count = 0;
  for (auto e : cg.edges()) {
    BOOST_CHECK(cg.contains(e));
    ++edge_count;
  }
  BOOST_CHECK_EQUAL(edge_count, cg.num_edges());

  BOOST_CHECK_EQUAL(cg.num_arguments(), 6);
  BOOST_CHECK_EQUAL(cg.count(a), 1);
  BOOST_CHECK_EQUAL(cg.count(d), 3);
  BOOST_CHECK_EQUAL(cg.count(f), 1);
  BOOST_CHECK_EQUAL(cg.count(make_arg("not_present")), 0);

  BOOST_CHECK(cg.cluster(v2) == Domain({b, c, d}));
  BOOST_CHECK(cg.separator(e12) == Domain({b}));
  BOOST_CHECK(cg.separator(e23) == Domain({c, d}));
  BOOST_CHECK(cg.separator(e24) == Domain({d}));
}

BOOST_FIXTURE_TEST_CASE(test_structure_queries_and_markov_network, Fixture) {
  BOOST_CHECK(cg.is_connected());
  BOOST_CHECK(cg.is_tree());
  BOOST_CHECK(cg.has_running_intersection());
  BOOST_CHECK(cg.is_triangulated());

  MarkovNetwork mn = cg.markov_network();
  BOOST_CHECK_EQUAL(mn.num_vertices(), 6);
  BOOST_CHECK(mn.contains(a, b));
  BOOST_CHECK(mn.contains(b, c));
  BOOST_CHECK(mn.contains(b, d));
  BOOST_CHECK(mn.contains(c, d));
  BOOST_CHECK(mn.contains(c, e));
  BOOST_CHECK(mn.contains(d, e));
  BOOST_CHECK(mn.contains(d, f));

  BOOST_CHECK_EQUAL(cg.remove_edge(e24), 1);
  BOOST_CHECK_EQUAL(cg.remove_edge(e24), 0);
  BOOST_CHECK(!cg.is_connected());
  BOOST_CHECK(!cg.has_running_intersection());
}

BOOST_FIXTURE_TEST_CASE(test_updates_and_removals, Fixture) {
  cg.update_cluster(v4, {d, e, f});
  BOOST_CHECK(cg.cluster(v4) == Domain({d, e, f}));

  cg.update_separator(e24, {d});
  BOOST_CHECK(cg.separator(e24) == Domain({d}));

  BOOST_CHECK_EQUAL(cg.remove_edge(v1, v3), 0);
  BOOST_CHECK_EQUAL(cg.remove_edge(v1, v2), 1);
  BOOST_CHECK_EQUAL(cg.num_edges(), 2);

  cg.clear_vertex(v2);
  BOOST_CHECK_EQUAL(cg.num_edges(), 0);
  BOOST_CHECK_EQUAL(cg.degree(v1), 0);
  BOOST_CHECK_EQUAL(cg.degree(v2), 0);
  BOOST_CHECK_EQUAL(cg.degree(v3), 0);
  BOOST_CHECK_EQUAL(cg.degree(v4), 0);

  cg.remove_vertex(v1);
  BOOST_CHECK_EQUAL(cg.num_vertices(), 3);

  cg.clear();
  BOOST_CHECK(cg.empty());
  BOOST_CHECK_EQUAL(cg.num_vertices(), 0);
  BOOST_CHECK_EQUAL(cg.num_edges(), 0);
}

BOOST_FIXTURE_TEST_CASE(test_find_and_intersection_queries, Fixture) {
  auto* cover_ab = cg.find_cluster_cover({a, b});
  BOOST_CHECK(cover_ab != nullptr);
  BOOST_CHECK(cg.cluster(cover_ab) == Domain({a, b}));

  auto sep_d = cg.find_separator_cover({d});
  BOOST_CHECK(cg.contains(sep_d));
  BOOST_CHECK(cg.separator(sep_d) == Domain({d}));

  auto* meets_f = cg.find_cluster_meets({f});
  BOOST_CHECK(meets_f != nullptr);
  BOOST_CHECK(cg.cluster(meets_f) == Domain({d, f}));

  auto sep_b = cg.find_separator_meets({b});
  BOOST_CHECK(cg.contains(sep_b));
  BOOST_CHECK(cg.separator(sep_b) == Domain({b}));

  std::unordered_set clusters_with_d = {v2, v3, v4};
  cg.intersecting_clusters({d}, [&](vertex_descriptor v) {
    BOOST_CHECK_EQUAL(clusters_with_d.erase(v), 1);
  });
  BOOST_CHECK(clusters_with_d.empty());


  std::unordered_set seps_with_d = {e23, e24};
  cg.intersecting_separators({d}, [&](edge_descriptor e) {
    BOOST_CHECK_EQUAL(seps_with_d.erase(e), 1);
  });
  BOOST_CHECK(seps_with_d.empty());
}

BOOST_FIXTURE_TEST_CASE(test_merge, Fixture) {
  vertex_descriptor retained = cg.merge(e12);
  BOOST_CHECK(retained != nullptr);
  BOOST_CHECK_EQUAL(cg.degree(retained), 2);
  BOOST_CHECK_EQUAL(cg.num_vertices(), 3);
  BOOST_CHECK_EQUAL(cg.num_edges(), 2);
  BOOST_CHECK(cg.cluster(retained) == Domain({a, b, c, d}));
  BOOST_CHECK(cg.is_connected());
  BOOST_CHECK(cg.is_tree());
  BOOST_CHECK(cg.has_running_intersection());
}

BOOST_FIXTURE_TEST_CASE(test_tree_traversals, Fixture) {
  using pair_type = std::pair<vertex_descriptor, vertex_descriptor>;

  std::unordered_set<vertex_descriptor> visited_pre = {v1};
  cg.pre_order_traversal(v1, [&](edge_descriptor e) {
    BOOST_CHECK(visited_pre.count(e.source()));
    BOOST_CHECK(visited_pre.insert(e.target()).second);
  });
  BOOST_CHECK_EQUAL(visited_pre.size(), cg.num_vertices());

  std::unordered_set<vertex_descriptor> visited_post;
  cg.post_order_traversal(v1, [&](edge_descriptor e) {
    BOOST_CHECK(visited_post.insert(e.source()).second);
    BOOST_CHECK(!visited_post.count(e.target()));
  });
  BOOST_CHECK_EQUAL(visited_post.size(), cg.num_vertices() - 1); // other than root

  std::unordered_set<edge_descriptor> visited_mpp;
  cg.mpp_traversal(v1, [&](edge_descriptor e) {
    edge_descriptor e_reversed = e.reverse();
    for (edge_descriptor in : cg.in_edges(e.source())) {
      if (in != e_reversed) {
        BOOST_CHECK(visited_mpp.count(in));
      }
    }
    BOOST_CHECK(visited_mpp.insert(e).second);
  });
  BOOST_CHECK_EQUAL(visited_mpp.size(), cg.num_edges() * 2);
}

BOOST_FIXTURE_TEST_CASE(test_reachable_and_subtree_cover_calls, Fixture) {
  Arg g = make_arg("g");
  Arg h = make_arg("h");
  vertex_descriptor v5 = cg.add_vertex({g, h});
  edge_descriptor e45 = cg.add_edge(v4, v5); // empty separator

  // reachable without propagating past empty
  cg.compute_reachable(false);
  BOOST_CHECK_EQUAL(cg.reachable(e12), Domain({a, b}));
  BOOST_CHECK_EQUAL(cg.reachable(e23), Domain({a, b, c, d, f}));
  BOOST_CHECK_EQUAL(cg.reachable(e24), Domain({a, b, c, d, e}));
  BOOST_CHECK_EQUAL(cg.reachable(e45), Domain());
  BOOST_CHECK_EQUAL(cg. reachable(e45.reverse()), Domain());
  BOOST_CHECK_EQUAL(cg.reachable(e24.reverse()), Domain({d, f}));
  BOOST_CHECK_EQUAL(cg.reachable(e23.reverse()), Domain({c, d, e}));
  BOOST_CHECK_EQUAL(cg.reachable(e12.reverse()), Domain({b, c, d, e, f}));

  // reachable with propagating past empty
  cg.compute_reachable(true);
  BOOST_CHECK_EQUAL(cg.reachable(e45), Domain({a, b, c, d, e, f}));
  BOOST_CHECK_EQUAL(cg. reachable(e45.reverse()), Domain({g, h}));

  // reachable with filter
  cg.compute_reachable(true, {a, g});
  BOOST_CHECK_EQUAL(cg.reachable(e12), Domain({a}));
  BOOST_CHECK_EQUAL(cg.reachable(e23), Domain({a, g}));
  BOOST_CHECK_EQUAL(cg.reachable(e24), Domain({a}));
  BOOST_CHECK_EQUAL(cg.reachable(e45), Domain({a}));
  BOOST_CHECK_EQUAL(cg.reachable(e45.reverse()), Domain({g}));
  BOOST_CHECK_EQUAL(cg.reachable(e24.reverse()), Domain({g}));
  BOOST_CHECK_EQUAL(cg.reachable(e23.reverse()), Domain());
  BOOST_CHECK_EQUAL(cg.reachable(e12.reverse()), Domain({g}));

  cg.mark_subtree_cover({a, g}, false);
  BOOST_CHECK(cg.marked(v1));
  BOOST_CHECK(!cg.marked(v2));
  BOOST_CHECK(!cg.marked(v3));
  BOOST_CHECK(!cg.marked(v4));
  BOOST_CHECK(cg.marked(v5));
  // only set internal marks, not change the structure
  BOOST_CHECK_EQUAL(cg.num_vertices(), 5);
  BOOST_CHECK_EQUAL(cg.num_edges(), 4);
  BOOST_CHECK(cg.is_tree());

  cg.mark_subtree_cover({a, g}, true);
  BOOST_CHECK(cg.marked(v1));
  BOOST_CHECK(cg.marked(v2));
  BOOST_CHECK(!cg.marked(v3));
  BOOST_CHECK(cg.marked(v4));
  BOOST_CHECK(cg.marked(v5));
  // only set internal marks, not change the structure
  BOOST_CHECK_EQUAL(cg.num_vertices(), 5);
  BOOST_CHECK_EQUAL(cg.num_edges(), 4);
  BOOST_CHECK(cg.is_tree());
}

BOOST_AUTO_TEST_CASE(test_triangulated_from_cliques) {
  Arg a = make_arg("ta");
  Arg b = make_arg("tb");
  Arg c = make_arg("tc");
  Arg d = make_arg("td");
  Arg e = make_arg("te");

  ClusterGraph jt;
  std::vector<ClusterGraph::vertex_descriptor> v =
    jt.triangulated({Domain({a, b, c}), Domain({b, c, d}), Domain({c, d, e})});

  BOOST_CHECK_EQUAL(jt.num_vertices(), 3);
  BOOST_CHECK_EQUAL(jt.num_edges(), 2);
  BOOST_CHECK(jt.contains(v[0], v[1]));
  BOOST_CHECK(jt.contains(v[1], v[2]));
  BOOST_CHECK(jt.is_connected());
  BOOST_CHECK(jt.is_tree());
  BOOST_CHECK(jt.has_running_intersection());
  BOOST_CHECK(jt.is_triangulated());
}

BOOST_AUTO_TEST_CASE(test_triangulated_from_markov_network) {
  Arg a = make_arg("a");
  Arg b = make_arg("b");
  Arg c = make_arg("c");
  Arg d = make_arg("d");
  Arg e = make_arg("e");
  Arg f = make_arg("f");
  std::vector<Arg> vertices = {a, b, c, d, e, f};
  std::vector<std::pair<Arg, Arg>> edges = {{a, b}, {b, c}, {c, d}, {a, d}, {d, e}, {d, f}, {e, f}};

  MarkovNetwork mn;
  for (Arg v : vertices) {
    mn.add_vertex(v);
  }
  for (auto [u, v] : edges) {
    mn.add_edge(u, v);
  }

  ClusterGraph jt;
  jt.triangulated(mn, MinDegreeStrategy());

  BOOST_CHECK(jt.num_vertices() > 0);
  BOOST_CHECK_EQUAL(jt.num_edges(), jt.num_vertices() - 1);
  BOOST_CHECK(jt.is_connected());
  BOOST_CHECK(jt.is_tree());
  BOOST_CHECK(jt.has_running_intersection());
  BOOST_CHECK(jt.is_triangulated());

  for (Arg v : vertices) {
    BOOST_CHECK(jt.count(v) >= 1);
  }
  for (auto [u, v] : edges) {
    BOOST_CHECK(jt.find_cluster_cover({u, v}));
  }
}
