...
namespace libgm { namespace experimental {
  template class logarithmic_table<var>;
  template class logarithmic_table<vec>;
} }
...
BOOST_AUTO_TEST_CASE(test_operators) {
  ...
  ltable f({x, y}, {0, 1, 2, 3});
  ltable g({y, z}, {1, 2, 3, 4, 5, 6});
  ltable h;
  h = f * g;
  ...
  ltable f1({x, y}, {0, 1, 2, 3});
  ltable f2({x, y}, {-2, 3, 0, 0});
...
BOOST_AUTO_TEST_CASE(test_sample) {
  ...
  var x = var::discrete(u, "x", 2);
  var y = var::discrete(u, "y", 3);
  ltable f({x, y}, {0, 0.1, 0.2, 0.3, 0.5, 0.6});
  f.normalize();
  std::mt19937 rng1;
  std::mt19937 rng2;
  std::mt19937 rng3;
